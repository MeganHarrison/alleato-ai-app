...
class MeetingInsightsGenerator:
    ...
    async def extract_insights_from_meeting(
        ...
        if not self._is_meeting(title, content):
            logger.info(f"Document {document_id} does not appear to be a meeting")
            return []
    ...
    def _is_meeting(self, title: str, content: str) -> bool:
        """Detect if content is a meeting."""
