-- AI Submittals Agent Database Schema (FIXED)
-- Updated to work with existing tables and follow current database patterns

-- NOTE: projects and users tables already exist, so we'll reference them instead of creating

-- Project team assignments (only if not exists)
-- Links existing users to existing projects with specific roles
CREATE TABLE IF NOT EXISTS project_users (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id INTEGER NOT NULL REFERENCES projects(id) ON DELETE CASCADE,  -- Changed to INTEGER to match existing projects table
...
CREATE TABLE specifications (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    project_id INTEGER NOT NULL REFERENCES projects(id) ON DELETE CASCADE,  -- Changed to INTEGER
...
-- Submittal-specific Notifications (extends existing notification patterns)
CREATE TABLE submittal_notifications (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    project_id INTEGER REFERENCES projects(id) ON DELETE CASCADE,  -- Changed to INTEGER
...
-- Analytics and Reporting (submittal-specific)
CREATE TABLE submittal_analytics_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    event_type VARCHAR(100) NOT NULL,
    project_id INTEGER REFERENCES projects(id),  -- Changed to INTEGER
...
-- Performance tracking for submittals
...
CREATE INDEX idx_submittal_documents_submittal_id ON submittal_documents(submittal_id);
CREATE INDEX idx_submittal_history_submittal_id ON submittal_history(submittal_id);
CREATE INDEX idx_submittal_notifications_user_id ON submittal_notifications(user_id);
CREATE INDEX idx_submittal_notifications_unread ON submittal_notifications(user_id, is_read);
...
CREATE INDEX idx_submittal_documents_text_search ON submittal_documents USING gin(to_tsvector('english', extracted_text));
...
SELECT 
    ...
    u.email as submitted_by_email,  -- Changed from name concatenation since users table structure is different
...
CREATE VIEW submittal_project_dashboard AS
SELECT 
    p.id,
    p.name,
    p.state as status,  -- Changed from p.status to p.state based on existing projects table
